        ...
        <div class="container-large-4">
            <div class="title-wrapper-large">
                <div class="wrap-v-small">
                    <div class="heading-detail">blog post</div>
                    <h1 class="h1-heading">{{$blog->name}}</h1>
                </div>
                <div class="blog-author-detail">
                    ...
                    <div class="blog-detail-wrapper">
                        <div class="wrap-h-regular align-c">
                            <div class="blog-avatar"><img src="{{asset('homePage')}}/images/Avatar-Blog_1Avatar-Blog.png" loading="lazy" alt="" class="avatar-image"></div>
                            <div>
                                <div class="paragraph-large text-weight-semibold">{{$blog->user->name}}</div>
                                <div class="wrap-h-x-small align-c">
                                    <div class="paragraph-large text-gray-500">{{$blog->created_at->format('Y M d')}}</div>
            ...
            <div class="blog-header-image"><img src="{{asset($blog->image)}}" loading="lazy" sizes="(max-width: 1439px) 90vw, 1278px" srcset="{{asset($blog->image)}} 500w, {{asset($blog->image)}} 800w, {{asset($blog->image)}} 2560w" alt="{{ $blog->name }}" class="image-cover"></div>
            <div class="container-x-small">
                <div class="wrap-v-x-large">
                    <div class="wrap-v-x-small">
                        <h5 class="h5-heading">{{$blog->name}}</h5>
                        <div class="sub-heading-small-2">{{$blog->description}}</div>
                    </div>
                    <p class="paragraph-large">{!! $blog->main_content !!}</p>
                </div>
